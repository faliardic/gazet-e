...
- Q10 owner-reset production revision active `gazet-e.edition-request.v2` / `gazet-e.edition.v2` contracts, summary-free lifecycle, Q06-to-Q08 direct visual facts, durable fenced image budgets, dedicated PostgreSQL mutation guard, versioned `350×500 mm` / `700×1000` physical page projection and local display-only ad reservation with backend/mobile integration olarak uygulandı. Q07 runtime imports/calls are zero; v1 schema/fixture, Q07 files, Q09 core, gesture/session code, provider/model settings and dependencies remain unchanged. Fake providers, isolated PostgreSQL, separate worker/API processes and full backend/mobile regressions provide deterministic evidence; independent review, live operation and real-device acceptance remain `PENDING`, so Q10 stays `ACTIVE` and Q11 `QUEUED`.
