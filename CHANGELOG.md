...
- Q08 AI visual editor pipeline; deterministic fact-grounded brief/safety classification, exact OpenAI Image API generation contract, local WebP/hash validation, bounded Terra semantic QA, fail-closed in-memory artifact ve exact-key success cache ile eklendi. Türkçe sensitive-event classifier sınırı konservatif çekim eşleşmeleriyle güçlendirildi; yeni production revision üzerindeki live gate `unsupported_visual_detail` nedeniyle fail-closed `unavailable` sonuçlandığından Q08 henüz COMPLETE değildir.
